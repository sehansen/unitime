...
def loadTestData(testNumber):
    alldata = [open(f) for f in glob.glob("./TestData/Test%02d/*.utt" % (testNumber))]
    ...
    M = {} # A dict with key = course, and value = minimum number of days that it should be spread across
    lecturer = {}
    ...
    C = {} # A dict with key = room, and value = number of students that can be in the room
    files['rooms'].readline()
    for line in files['rooms']:
        Room, Capacity = tuple(line.split(" "))
        C[Room] = int(Capacity)

    curric = {} # A dict with key = couse, and value = list of curriculae it belongs to
